// src/webhooks/ssrf-guard.ts -- SSRF guard for outbound webhook delivery.
//
...
//
// The pinning works because the same validated address list returned by the
// custom `lookup` is what undici actually connects to -- there is no second
// resolution between the check and the connect. Literal-IP hosts skip DNS in
// undici, so `assertPublicUrl` also checks literal IPs up front (defense in depth).
