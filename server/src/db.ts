...
export function initDatabase(): Database.Database {
  ...
  db.exec(`
    ...
    -- BRRRR + STR real estate operating system. All event timestamps in
    -- milliseconds (CLAUDE.md hard rule). Calendar dates use TEXT YYYY-MM-DD
    -- so ordering still works lexicographically. project_id required on
    -- every row for multi-project scoping per resolveProjectScope().
    -- Schema mirrored to bot DB (src/db.ts) for collector write access.
    -- =====================================================================
    ...
    -- CRITICAL: STR loophole + REPS audit-defense ledger. Every entry is
    -- evidence in an IRS audit. Activity must be specific (not "managed
    -- property"); hours must be plausible; participant + counted_for must
    -- match the entity's ownership structure (see legal-shield 5% rule).
    ...
    -- Non-RE asset log. MANUAL ENTRY ONLY (no API pulls per project scope).
    -- EXCLUDED from public OSS mirror via the sanitizer.
  ...
  seedBrokerProperties()
  logger.info({ path: DB_PATH }, 'Database initialized')
  return db
}

/**
 * Seed Paw Broker anchor properties (one-time, idempotent on count check).
 * 1932 W Shunk: South Philly LTR, BRRRR-cycle complete. Lat/lng from plan
 * verification step. Acquisition price + cost_basis + current_arv left
 * NULL until owner fills via the dashboard.
 *
 * 114 Michigan: primary residence with basis raised by major repairs +
 * 3-car garage + 2nd-floor attic build. Three improvements rows seeded
 * with NULL costs (owner fills later). Zip + city deferred per
 * SESSION-HANDOFF.md open question.
 */
function seedBrokerProperties(): void {
  const count = (db.prepare("SELECT COUNT(*) AS c FROM properties WHERE project_id = 'broker'").get() as { c: number }).c
  if (count > 0) return
  ...
  const tx = db.transaction(() => {
    ...
    // Improvements that raised 114 Michigan basis. Costs NULL until owner fills.
    insertImp.run({
  ...
  tx()
  logger.info({ properties: 2, improvements: 3 }, 'Seeded Paw Broker anchor properties')
}
